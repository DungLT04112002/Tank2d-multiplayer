...
public class CollisionChecker {
    ...
    public boolean objectCheck(Player player) {
        ...
        }else if (gamePanel.chooseMap == 3) {
            for (int i = 0; i < gamePanel.getMap3().getObjectManager().MAX_OBJECT; ++i) {
                if (gamePanel.getMap3().getObjectManager().getObjects()[i].inUse()) {
                    if (player.getHitBox()
                            .isCollision(gamePanel.getMap3().getObjectManager().getObjects()[i].getHitBox())) {
                        return true;
                    }
                }
            }
        }else if (gamePanel.chooseMap == 4) {
            for (int i = 0; i < gamePanel.getMap4().getObjectManager().MAX_OBJECT; ++i) {
                if (gamePanel.getMap4().getObjectManager().getObjects()[i].inUse()) {
                    if (player.getHitBox()
                            .isCollision(gamePanel.getMap4().getObjectManager().getObjects()[i].getHitBox())) {
                        return true;
                    }
                }
            }
            ...
            for (int i = 0; i < gamePanel.getMap3().getObjectManager().MAX_OBJECT; ++i) {
                if (gamePanel.getMap3().getObjectManager().getObjects()[i].inUse()) {
                    if (bullet.getHitBox()
                        ...
                        gamePanel.playSE(4);
                        bullet.delete();
                        gamePanel.getMap3().getObjectManager().getObjects()[i].destroy();
            ...
            for (int i = 0; i < gamePanel.getMap4().getObjectManager().MAX_OBJECT; ++i) {
                if (gamePanel.getMap4().getObjectManager().getObjects()[i].inUse()) {
                    if (bullet.getHitBox()
                        ...
                        gamePanel.playSE(4);
                        bullet.delete();
                        gamePanel.getMap4().getObjectManager().getObjects()[i].destroy();
